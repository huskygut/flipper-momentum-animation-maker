...
class FlipperMomentumGifMakerApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.pack_name = tk.StringVar(value="PackName")
        ...
        self.preview_mode = tk.StringVar(value="accurate")
        self.create_zip = tk.BooleanVar(value=True)
        self.hold_last_frame = tk.IntVar(value=0)
        ...
        self.image_processor = ImageProcessor(
            ...
            brightness_getter=lambda: self._safe_float(self.brightness, 1.0),
            sharpen_getter=lambda: self._safe_float(self.sharpen, 0.0),
        )
        self.manifest_builder = ManifestBuilder(
            frame_rate_getter=lambda: self._safe_int(self.frame_rate, 5),
            ...
            hold_last_frame_getter=lambda: self._safe_int(self.hold_last_frame, 0),
        )
    ...
    def _release_all_frames(self) -> None:
    ...
    def _build_ui(self) -> None:
        ...
        self.play_button = tk.Button(top, text="Play", command=self.toggle_playback, width=14)
        ...
        self._add_spinbox(options, "Frame rate", self.frame_rate, 1, 60, 2, 4)
        self._add_spinbox(options, "Duration", self.duration, 1, 5000, 2, 6)
        self._add_spinbox(options, "Passive frames", self.passive_frames, 1, 9999, 4, 0)
        self._add_spinbox(options, "Active cycles", self.active_cycles, 1, 9999, 4, 2)
        self._add_spinbox(options, "Active cooldown", self.active_cooldown, 0, 9999, 4, 4)
        self._add_spinbox(options, "Hold last frame", self.hold_last_frame, 0, 9999, 4, 6)

        self._add_spinbox(options, "Min butthurt", self.min_butthurt, 0, 18, 6, 0)
        self._add_spinbox(options, "Max butthurt", self.max_butthurt, 0, 18, 6, 2)
        self._add_spinbox(options, "Min level", self.min_level, 0, 30, 6, 4)
        self._add_spinbox(options, "Max level", self.max_level, 0, 30, 6, 6)
        self._add_spinbox(options, "Weight", self.weight, 1, 100, 6, 8)

        tk.Label(options, text="5 fps is a solid default. Very high frame rates can ghost on-device.", fg=THEME_ACCENT).grid(
            row=3, column=4, columnspan=5, sticky="w", padx=6, pady=(0, 6)
        )

        tk.Label(options, text="Fit mode").grid(row=8, column=0, sticky="w", padx=6, pady=(8, 0))
        fit_menu = tk.OptionMenu(options, self.fit_mode, "contain", "cover")
        fit_menu.grid(row=9, column=0, sticky="w", padx=6)

        tk.Label(options, text="Preview mode").grid(row=8, column=2, sticky="w", padx=6, pady=(8, 0))
        preview_menu = tk.OptionMenu(options, self.preview_mode, "accurate", "soft")
        preview_menu.grid(row=9, column=2, sticky="w", padx=6)

        tk.Checkbutton(options, text="Also make ZIP", variable=self.create_zip).grid(
            row=9, column=4, sticky="w", padx=6
        )
        ...
        self.preview_canvas_image_id = self.preview_canvas.create_image(
            PREVIEW_CANVAS_SIZE[0] // 2,
            PREVIEW_CANVAS_SIZE[1] // 2,
            image="",
        )
    ...
    def _wire_preview_refresh(self) -> None:
        preview_vars = [self.threshold, self.contrast, self.brightness, self.sharpen, self.fit_mode, self.preview_mode]
        info_vars = [
            self.threshold,
            self.contrast,
            self.brightness,
    ...
    def refresh_preview(self) -> None:
        ...
        try:
            ...
            preview = self.image_processor.build_preview_image(frame, self.preview_mode.get())
            if preview.size != PREVIEW_CANVAS_SIZE:
                resized_preview = preview.resize(PREVIEW_CANVAS_SIZE, Image.LANCZOS)
                try:
                    preview.close()
                except Exception:
                    pass
                preview = resized_preview
        ...
        self.status_label.config(
            text=(
                ...
                f"{WIDTH}x{HEIGHT}   {max(1, self._safe_int(self.frame_rate, 5))} fps\n"
                f"Threshold {self._safe_int(self.threshold, 140)}   Contrast {self._safe_float(self.contrast, 2.2):.1f}   "
                f"Brightness {self._safe_float(self.brightness, 1.0):.1f}   Sharpen {self._safe_float(self.sharpen, 0.0):.1f}   Preview {self.preview_mode.get()}"
            )
        )
    ...
    def refresh_info_text(self) -> None:
        ...
        hold_last = max(0, self._safe_int(self.hold_last_frame, 0))
        ...
        text = (
            ...
            f"Brightness: {self._safe_float(self.brightness, 1.0):.1f}\n"
            f"Sharpen: {self._safe_float(self.sharpen, 0.0):.1f}\n"
            f"Frame rate: {self._safe_int(self.frame_rate, 5)} fps (5 is a strong default)\n"
            f"Hold last frame repeats: {hold_last}\n"
    ...
    def _play_step(self) -> None:
        ...
        self.current_frame_index = (self.current_frame_index + 1) % active_count
        ...
        delay = max(20, int(1000 / max(1, self._safe_int(self.frame_rate, 5))))
    ...
    def export_pack(self) -> None:
        ...
        try:
            result = self.exporter.export_pack(
                ...
                expected_frame_count=self.active_frame_count(),
            )
        except Exception as exc:
        ...
        lines = [
            ...
            f"Sharpen: {self._safe_float(self.sharpen, 0.0):.1f}",
            f"Frame rate: {self._safe_int(self.frame_rate, 5)} fps (5 is a strong default)",
            f"Hold last frame repeats: {self._safe_int(self.hold_last_frame, 0)}",
