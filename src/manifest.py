...
class ManifestBuilder:
    def __init__(
        ...
        self.hold_last_frame_getter = hold_last_frame_getter
    ...
    def build_meta(self, frame_count: int) -> str:
        ...
        hold_last = max(0, int(self.hold_last_frame_getter()))
        frame_order_ids = list(range(frame_count))
        if frame_count > 0 and hold_last > 0:
            frame_order_ids.extend([frame_count - 1] * hold_last)
        frame_order = " ".join(str(i) for i in frame_order_ids)
        return (
